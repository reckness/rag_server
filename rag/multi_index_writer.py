...
def _ensure_index(es, index_name: str, mapping: dict):
    ...
    if not es.indices.exists(index=index_name):
        ...
        properties = current[index_name]["mappings"].get("properties", {})
        missing = {
            field: definition
            for field, definition in mapping["mappings"]["properties"].items()
            if field not in properties
        }
        if missing:
            es.indices.put_mapping(index=index_name, body={"properties": missing})
            logger.info(f"Updated index mapping: {index_name}, fields={list(missing.keys())}")
...
def write_to_three_indices(
    ...
    doc_summary = (data.get("summary") or "").strip()
    ...
    # ==================== 1. doc_index ====================
    doc_searchable_text = "\n".join([p for p in [doc_name, doc_summary] if p])
    ...
    doc_record = {
        ...
        "searchable_text": doc_searchable_text,
...
def main():
    parser = argparse.ArgumentParser(description="Write structure JSON to doc/chapter/chunk indices")
    parser.add_argument("json_path", help="Path to structure JSON generated by run_pdf_to_md_chunked.py")
    parser.add_argument("--doc-id", default="", help="Document ID")
    parser.add_argument("--kb-id", default="test01", help="Knowledge Base ID")
    parser.add_argument("--fd-id", default="test01", help="Folder ID")
    parser.add_argument("--doc-title", default="", help="Document title")
    args = parser.parse_args()

    doc_id = args.doc_id or os.path.splitext(os.path.basename(args.json_path))[0]
    ...
    print(f"indexed doc_id={doc_id}, kb_id={args.kb_id}, chunks={chunk_num}")
